...
This command uses a native dotnet template `webapi` to create a web API project that generates fake weather data. Note that if you compared the generated files with this Github repository, you will find an extra file named "Dockerfile", which is not generated by the previous command and whose importance will be explained later in this tutorial.
